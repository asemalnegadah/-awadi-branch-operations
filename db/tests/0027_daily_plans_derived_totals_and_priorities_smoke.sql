...
BEGIN
  ...
  SELECT manager_id, id, manager_id
  FROM roles
  ...
  INSERT INTO sales_representatives (
    ...
    'PLAN-DERIVED-REP-001',
    'مندوب إجماليات الخطة',
    'RETAIL',
    'ACTIVE',
    manager_id,
    manager_id
  ...
  INSERT INTO customers (customer_number, trade_name_ar, created_by, updated_by)
  VALUES ('PLAN-DERIVED-C3', 'عميل غير مكلف للمندوب', manager_id, manager_id)
  RETURNING id INTO unassigned_customer_id;
  ...
  INSERT INTO customer_route_assignments (
    ...
    (unassigned_customer_id, route_id_value, 'مسار العميل غير المكلف', manager_id, manager_id);
  ...
  BEGIN
    INSERT INTO daily_plan_candidates (
      ...
      true,
      3,
      'عميل خارج نطاق المندوب.',
      '[]'::jsonb,
      '{}'::jsonb
    );
    RAISE EXCEPTION 'expected out-of-scope candidate to fail';
  EXCEPTION WHEN raise_exception THEN
    IF SQLERRM = 'expected out-of-scope candidate to fail' THEN RAISE; END IF;
    IF position('daily plan candidate customer is not assigned to the representative at cutoff' IN SQLERRM) = 0 THEN
      RAISE EXCEPTION 'unexpected candidate scope error: %', SQLERRM;
    END IF;
  END;
