...
export async function loginPostgres(
  ...
  const maxEmailAttempts = configuration.maxEmailAttemptsPer15Minutes ?? 10;
  const maxIpAttempts = configuration.maxIpAttemptsPer15Minutes ?? 30;
  ...
  if (
    ...
    await recordFailure(sql, normalizedEmail, null, "RATE_LIMITED", context);
  ...
  const outcome = await sql.begin<LoginTransactionOutcome>(async (transaction) => {
    ...
    if (!user) {
      ...
      await recordFailure(transaction, normalizedEmail, null, "INVALID_CREDENTIALS", context);
      return { ok: false, code: "INVALID_CREDENTIALS" };
    }

    if (user.status !== "ACTIVE" || !user.password_hash) {
      await verifyPassword(input.password, await dummyPasswordHash);
      await recordFailure(transaction, normalizedEmail, user.id, "ACCOUNT_DISABLED", context);
      return { ok: false, code: "ACCOUNT_DISABLED" };
    }

    if (user.locked_until && new Date(user.locked_until).getTime() > Date.now()) {
      await recordFailure(transaction, normalizedEmail, user.id, "ACCOUNT_LOCKED", context);
    ...
    if (!validPassword) {
      ...
      await transaction`
        ...
        WHERE id = ${user.id}
      `;
      await recordFailure(transaction, normalizedEmail, user.id, shouldLock ? "ACCOUNT_LOCKED" : "INVALID_CREDENTIALS", context);
      return { ok: false, code: shouldLock ? "ACCOUNT_LOCKED" : "INVALID_CREDENTIALS" };
    }
...
async function isLoginRateLimited(
  ...
  const emailRows = await sql<{ count: number }[]>`
    SELECT COUNT(*)::integer AS count FROM auth_login_attempts
    WHERE normalized_email = ${normalizedEmail}
      AND succeeded = false
      AND occurred_at > now() - interval '15 minutes'
  `;
  if ((emailRows[0]?.count ?? 0) >= maxEmailAttempts) return true;
  if (!context.ipAddress) return false;
  const ipRows = await sql<{ count: number }[]>`
    SELECT COUNT(*)::integer AS count FROM auth_login_attempts
    WHERE ip_address = ${context.ipAddress}
      AND succeeded = false
      AND occurred_at > now() - interval '15 minutes'
  `;
  return (ipRows[0]?.count ?? 0) >= maxIpAttempts;
}
...
async function recordFailure(
  ...
  await sql`
    INSERT INTO auth_login_attempts (
      normalized_email, user_id, succeeded, failure_reason, request_id, ip_address, user_agent
    ) VALUES (
      ${normalizedEmail}, ${userId}, false, ${failureCode}, ${context.requestId},
  ...
  await sql`
    INSERT INTO audit_logs (
      actor_user_id, actor_type, action, resource_type, resource_id,
      request_id, ip_address, user_agent, result, metadata
    ) VALUES (
      ${userId}, 'USER', 'auth.login', 'AUTHENTICATION', ${normalizedEmail},
      ${context.requestId}, ${context.ipAddress}, ${context.userAgent}, 'DENIED',
