...
import { reverseCreditExceptionUsage } from "@/lib/risk/usage-service";
import { parseReverseCreditExceptionUsageInput } from "@/lib/risk/usage-validation";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await reverseCreditExceptionUsage(
      getDatabaseClient(),
      parseReverseCreditExceptionUsageInput(await readCreditRiskJson(request)),
