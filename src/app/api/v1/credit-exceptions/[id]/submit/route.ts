...
import { submitCreditException } from "@/lib/risk/service";
import { parseDecisionTransitionInput, parseRiskId } from "@/lib/risk/validation";
export const runtime = "nodejs";
type RouteContext = { readonly params: Promise<{ readonly id: string }> };
export async function POST(request: NextRequest, routeContext: RouteContext) {
  const authorization = await authorizeCreditRiskApiRequest(request, "credit_exceptions.propose", true);
  if (!authorization.ok) return authorization.response;
  try {
    const { id } = await routeContext.params;
    const result = await submitCreditException(getDatabaseClient(), parseRiskId(id), parseDecisionTransitionInput(await readCreditRiskJson(request)), buildCreditRiskCommandContext(authorization, request));
