...
import { rejectCreditException } from "@/lib/risk/service";
...
export async function POST(request: NextRequest, routeContext: RouteContext) {
  ...
  try {
    ...
    const result = await rejectCreditException(getDatabaseClient(), parseRiskId(id), parseDecisionTransitionInput(await readCreditRiskJson(request)), buildCreditRiskCommandContext(authorization, request));
