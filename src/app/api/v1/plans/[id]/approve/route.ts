...
import { approveDailyPlan } from "@/lib/plans/service";
import { parseDailyPlanId, parseDailyPlanTransitionInput } from "@/lib/plans/validation";
export const runtime = "nodejs";
type RouteContext = { readonly params: Promise<{ readonly id: string }> };
export async function POST(request: NextRequest, routeContext: RouteContext) {
  const authorization = await authorizeDailyPlanApiRequest(request, "plans.approve", true);
  if (!authorization.ok) return authorization.response;
  try {
    const { id } = await routeContext.params;
    const result = await approveDailyPlan(getDatabaseClient(), parseDailyPlanId(id), parseDailyPlanTransitionInput(await readDailyPlanJson(request)), buildDailyPlanCommandContext(authorization, request));
