...
import { completeDailyPlan } from "@/lib/plans/service";
...
export async function POST(request: NextRequest, routeContext: RouteContext) {
  ...
  try {
    ...
    const result = await completeDailyPlan(getDatabaseClient(), parseDailyPlanId(id), parseDailyPlanTransitionInput(await readDailyPlanJson(request)), buildDailyPlanCommandContext(authorization, request));
