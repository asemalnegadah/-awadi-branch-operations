...
import { submitDailyPlan } from "@/lib/plans/service";
...
export async function POST(request: NextRequest, routeContext: RouteContext) {
  ...
  try {
    ...
    const result = await submitDailyPlan(getDatabaseClient(), parseDailyPlanId(id), parseDailyPlanTransitionInput(await readDailyPlanJson(request)), buildDailyPlanCommandContext(authorization, request));
