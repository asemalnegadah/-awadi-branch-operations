...
import { submitCreditRestriction } from "@/lib/risk/service";
import { parseDecisionTransitionInput, parseRiskId } from "@/lib/risk/validation";
export const runtime = "nodejs";
type RouteContext = { readonly params: Promise<{ readonly id: string }> };
export async function POST(request: NextRequest, routeContext: RouteContext) {
  const authorization = await authorizeCreditRiskApiRequest(request, "credit_restrictions.propose", true);
  if (!authorization.ok) return authorization.response;
  try {
    const { id } = await routeContext.params;
    const result = await submitCreditRestriction(getDatabaseClient(), parseRiskId(id), parseDecisionTransitionInput(await readCreditRiskJson(request)), buildCreditRiskCommandContext(authorization, request));
