...
import { rejectCreditRestriction } from "@/lib/risk/service";
...
export async function POST(request: NextRequest, routeContext: RouteContext) {
  ...
  try {
    ...
    const result = await rejectCreditRestriction(getDatabaseClient(), parseRiskId(id), parseDecisionTransitionInput(await readCreditRiskJson(request)), buildCreditRiskCommandContext(authorization, request));
