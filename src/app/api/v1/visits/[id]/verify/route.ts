...
import { verifyFieldVisit } from "@/lib/visits/service";
import { parseFieldVisitTransition } from "@/lib/visits/validation";
export const runtime = "nodejs";
type RouteContext = { readonly params: Promise<{ readonly id: string }> };
export async function POST(request: NextRequest, routeContext: RouteContext) {
  const authorization = await authorizeFieldVisitApiRequest(request, "visits.verify", true);
  if (!authorization.ok) return authorization.response;
  try {
    const { id } = await routeContext.params;
    const data = await verifyFieldVisit(getDatabaseClient(), parseFieldVisitId(id), parseFieldVisitTransition(await readFieldVisitJson(request)), buildFieldVisitCommandContext(authorization, request));
