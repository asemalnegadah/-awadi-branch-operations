...
import { parseFieldVisitCheckout, parseFieldVisitId } from "@/lib/visits/route-validation";
import { checkOutFieldVisit } from "@/lib/visits/service";
...
export async function POST(request: NextRequest, routeContext: RouteContext) {
  ...
  try {
    ...
    const data = await checkOutFieldVisit(getDatabaseClient(), parseFieldVisitId(id), parseFieldVisitCheckout(await readFieldVisitJson(request)), buildFieldVisitCommandContext(authorization, request));
