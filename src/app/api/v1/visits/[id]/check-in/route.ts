...
import { checkInFieldVisit } from "@/lib/visits/service";
import { parseFieldVisitLocation } from "@/lib/visits/validation";
...
export async function POST(request: NextRequest, routeContext: RouteContext) {
  ...
  try {
    ...
    const data = await checkInFieldVisit(getDatabaseClient(), parseFieldVisitId(id), parseFieldVisitLocation(await readFieldVisitJson(request)), buildFieldVisitCommandContext(authorization, request));
