...
import { returnFieldVisit } from "@/lib/visits/service";
...
export async function POST(request: NextRequest, routeContext: RouteContext) {
  ...
  try {
    ...
    const data = await returnFieldVisit(getDatabaseClient(), parseFieldVisitId(id), parseFieldVisitTransition(await readFieldVisitJson(request)), buildFieldVisitCommandContext(authorization, request));
